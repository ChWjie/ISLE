...
4. 如果只查看中国地图和内置案例，可直接点击应用页「预览」或工作空间顶部「运行」。Cloud Studio 会读取 `.vscode/preview.yml`，通过统一的预览进程依次准备数据库并启动：

   - SQLite：从 `database/law_game.db` 创建独立运行副本（无需数据库进程）
   - 后端 API：`8000`，健康检查通过后再启动前端
   - 前端：`5173`（主预览端口），并检查 `/api/v1/events` 代理
...
脚本会安装依赖、测试数据库连接、创建缺失的数据表，并在后台启动后端 `8000` 与前端 `5173`。应用页的一键「预览」固定使用仓库中的最终 SQLite 数据；终端脚本则继续读取 `backend/.env`，便于需要时连接外部数据库。
